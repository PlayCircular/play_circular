...
(function(window) {
  return {
    ...
    init: function init(id, op, callback) {
      ...
      if(this.adds)if(this.adds.length>2)this.adds=undefined;
      if (!this.adds) {
   ...
   //    console.log(that.id,x, y, row, col, data, colorSet, colorIndex)
    ...
    add: function(op, callback){
      var that = this;console.log(this.id, this.adds)
      this.adds.push([this.id, op, callback]);
      this.init(this.id, this.op, function(){
        that.adds.push([that.id, op, callback]);
        that.preProcessing (op, callback);
        that.adds = undefined;
      });
      return this;
