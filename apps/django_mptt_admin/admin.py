...
class DjangoMpttAdmin(admin.ModelAdmin):
	...
	def get_tree_data(self, qs, max_level, request):
		...
		if request.user.is_superuser:
			...
			if len(grupos_administrados) > 0:
				...
				qs = qs.filter(grupo__in=grupos_administrados,superadmin=False)
			else:
	...
	def tree_json_view(self, request):
		...
		tree_data = self.get_tree_data(qs, max_level, request)
